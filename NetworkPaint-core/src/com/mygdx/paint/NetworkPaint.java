...
public class NetworkPaint extends ApplicationAdapter {
	...
	public void render () {
		...
		if(current != null){ //Musimy sprawdzic czy przypadkiem PosTab nie jest pusty(null) bo inaczej wywali nam NullPointerException
			...
			if(previous != null){ //Sprawdzenie, czy nie jest to pierwszy punkt
				if(current.type == 2&&previous.type!=0){ //Jesli mysz jest przeciagana
					...
					current.draw(shapeRenderer,previous.x,height-previous.y,height);
		...
		if(current.type==5){
			Shape bob = new Point(current.x, current.y, current.brush_size, current.type, current.r, current.g, current.b,current.id);
			bob.draw(shapeRenderer,200,height-200,height);
			Shape bob1 = new Rect(current.x, current.y, current.brush_size, current.type, current.r, current.g, current.b,current.id);
